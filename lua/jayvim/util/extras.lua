...
M.sources = {
  {
    name = "JayVim",
    desc = "JayVim extras",
    module = "jayvim.plugins.extras",
  },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("jayvim.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #JayVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = JayVim.find_root(source.module)
    if root then
      JayVim.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  ...
  return {
    ...
    managed = vim.tbl_contains(JayVim.config.json.data.extras, modname)
      or not enabled,
    plugins = plugins,
    optional = optional,
  }
...
function X.new()
  ...
  self.float = Float.new({ title = "JayVim Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      JayVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, JayVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(JayVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(JayVim.config.json.data.extras)
      JayVim.json.save()
      JayVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart JayVim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("JayVim Extras", "LazyH1"):nl():nl()
  self.text
    :append(
      "This is a list of all enabled/disabled JayVim extras.",
      "LazyComment"
    )
    :nl()
    :append(
...
function X:extra(extra)
  if not extra.managed then
    ...
    elseif vim.tbl_contains(JayVim.plugin.core_imports, extra.module) then
      self:diagnostic({
  ...
  if extra.recommended then
    self.text
      :append(" ")
      :append(LazyConfig.options.ui.icons.favorite or " ", "LazyCommit")
  end
  if extra.source.name ~= "JayVim" then
    self.text:append(" "):append(
      LazyConfig.options.ui.icons.event .. extra.source.name,
      "LazyReasonEvent"
    )
  end
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^jayvim.plugins.extras.", "")
    self.text
      :append(" ")
      :append(LazyConfig.options.ui.icons.plugin .. import, "LazyReasonStart")
  end
  for _, plugin in ipairs(extra.plugins) do
    self.text
      :append(" ")
      :append(LazyConfig.options.ui.icons.plugin .. plugin, "LazyReasonPlugin")
  end
  for _, plugin in ipairs(extra.optional) do
    self.text
      :append(" ")
      :append(LazyConfig.options.ui.icons.plugin .. plugin, "LazyReasonRequire")
  end
  if extra.desc then
...
function X:section(opts)
  ...
  self.text
    :nl()
    :append(opts.title .. ":", "LazyH2")
    :append(" (" .. #extras .. ")", "LazyComment")
    :nl()
