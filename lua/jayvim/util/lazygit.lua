...
M.theme_path = JayVim.norm(vim.fn.stdpath("cache") .. "/lazygit-theme.yml")
...
function M.open(opts)
  if vim.g.lazygit_theme ~= nil then
    JayVim.deprecate("vim.g.lazygit_theme", "vim.g.lazygit_config")
  end
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        vim.env.LG_CONFIG_FILE =
          JayVim.norm(M.config_dir .. "/config.yml" .. "," .. M.theme_path)
      else
  ...
  return JayVim.terminal(cmd, opts)
end
...
function M.get_color(v)
  ...
  local color = {}
  if v.fg then
    color[1] = JayVim.ui.color(v.fg)
  elseif v.bg then
    color[1] = JayVim.ui.color(v.bg, true)
  end
  if v.bold then
...
function M.blame_line(opts)
  ...
  local root = JayVim.root.detectors.pattern(0, { ".git" })[1] or "."
  local cmd = {
    "git",
    "-C",
    root,
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      JayVim.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return JayVim.error("No git remotes found")
