...
function M.formatexpr()
  if JayVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  JayVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    JayVim.warn("No formatter available", { title = "JayVim" })
  end
end
...
function M.health()
  ...
  local has_extra =
    vim.tbl_contains(Config.spec.modules, "jayvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    JayVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in JayVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `jayvim.plugins.extras.lsp.none-ls` extra,",
